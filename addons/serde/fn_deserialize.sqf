...
private _deserialize_single = {
    ...
    if (_type == "HA") then {
        ...
        } forEach ([_value_string, _object_separator, _object_start, _object_end] call FNC(serde,_splitStringDelimited));
    };
    if (_type == "AR") then {
        ...
        } forEach ([_value_string, _array_separator, _array_start, _array_end] call FNC(serde,_splitStringDelimited));
    };
    if (_type == "BO") then {
...
private _deserialize = {
    ...
    if (count _bits == 2) then {
        ...
        _dictionary set [_key, _value];
    };
};
["HA", _string] call _deserialize_single
