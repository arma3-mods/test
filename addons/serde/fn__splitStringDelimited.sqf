...
private _bits = [_string, _delimiter] call FNC(serde,_splitString);
private _level = 0;
private _result = [];
{
    if (_level == 0) then {
    ...
    _level = _level + count ([_x, _starter] call FNC(serde,_splitString)) - count ([_x, _ender] call FNC(serde,_splitString));
} forEach _bits;
_result
